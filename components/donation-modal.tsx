...
export default function DonationModal() {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 max-w-md rounded-lg bg-white p-6 shadow-lg animate-fade-up">
      <button onClick={() => setIsVisible(false)} className="absolute right-2 top-2 text-gray-500 hover:text-gray-700">
        <X className="h-4 w-4" />
      </button>
      <div className="mb-4">
        <h3 className="text-xl font-bold">Support Our Campaign</h3>
        <p className="mt-2 text-gray-600">Help us build a better future. Your contribution makes a difference.</p>
      </div>
      ...
      <Button className="mt-4 w-full bg-primary hover:bg-primary/90">OTHER AMOUNT</Button>
    </div>
  )
}
